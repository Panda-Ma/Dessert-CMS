...
import {getDessertNum, getNum} from "/@/api/home";
  ...
  setup: function () {
    ...
    const state = reactive({
      homeOne: [
        {
          num1: '-1', // 数量
          num2: '', //一个正负数，增加或下降幅度，不需要用到
          num3: '点心数量',  // 类别、科目名称
          num4: 'fa fa-address-book',
          color1: '#FF6462',
    ...
    onMounted(() => {
      getNum().then((res: any) => {
        state.homeOne[0].num1 = res.data.dessertNum
        state.homeOne[1].num1 = res.data.orderNum
        state.homeOne[2].num1 = res.data.userNum
      })
      initEchartsResize();
    });
    // 由于页面缓存原因，keep-alive
